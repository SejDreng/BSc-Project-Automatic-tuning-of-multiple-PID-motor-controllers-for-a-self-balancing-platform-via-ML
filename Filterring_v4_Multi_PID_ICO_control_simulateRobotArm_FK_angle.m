function simulateRobotArm(N, L, t, init_pos, H_poisson)
    ...
    disturbance_total_angle = 0.1 * pi;
    disturbance_duration = 1.0;                  % in seconds
    disturbance_interval = 10.0;                 % in seconds
    disturbance_steps = round(disturbance_duration / dt);  % number of iterations over which disturbance is applied
    disturbance_period = round(disturbance_interval / dt); % interval between disturbances
    
    % Smooth ramp using half-cosine
    disturbance_ramp = disturbance_total_angle * 0.5 * (1 - cos(linspace(0, pi, disturbance_steps)));
    ...
    % Maintain a buffer of past X values (size of buffer = filter length)
    ...
    for iter = 1:max_iterations
        ...
        X0 = prev_error;  % Force row vector
        ...
        % Low-pass filtered derivative (Just a constant dampening in this iteration.)
                % Update X0 buffer
        if size(X0_buffer, 1) < buffer_size
            X0_buffer = [X0; X0_buffer];
        else
            X0_buffer = [X0; X0_buffer(1:end-1, :)];
        end
        
        % Compute filtered X0
        X0 = arrayfun(@(i)conv(X0_buffer(1:min(iter, buffer_size), i), H_poisson, 'same'), 1:N, 'UniformOutput', false);
        X0 = cellfun(@(c) c(end), X0);
        
        raw_derivative = (error - prev_error) / dt;
        ICO_derivative = (X0 - X0_prev) / dt;
        ...
        if size(X1_kd_buffer, 1) < buffer_size
            X1_kd_buffer = [X1_kd; X1_kd_buffer];
        else
            X1_kd_buffer = [X1_kd; X1_kd_buffer(1:end-1, :)];
        end
        
        X1_kd_filtered = arrayfun(@(i)conv(X1_kd_buffer(1:min(iter, buffer_size), i), H_poisson, 'same'), 1:N, 'UniformOutput', false);
        X1_kd_filtered = cellfun(@(c) c(end), X1_kd_filtered);
        
        dw1_kd = mu * (X1_kd_filtered .* ICO_derivative);
        w1_kd = w1_kd + dw1_kd;
        Kd = (X0 * w0) + (X1_kd_filtered .* w1_kd);
        ...
        % Check if we are within a disturbance window
        mod_iter = mod(iter, disturbance_period);
        if mod_iter > 0 && mod_iter <= disturbance_steps
            D = disturbance_ramp(mod_iter) * ones(1, N);
        elseif mod_iter == 0
            % Optional: for exact multiples, force zero or reset
            D = zeros(1, N);
        end
...
function plotRobot(joint_angles, arm_lengths, N, prev_angles, gif_filename, save_gif, iter)
    ...
    num_steps = 2; % Number of interpolation steps, increase for smoother animation
...
% Generate filter
fil_len = 51;                       % Filter length
low_lambda = 20.0;               % Controls high-pass decay
high_lambda = 1.5;               % Controls low-pass decay
...
t = 0*pi; %replace with target

init_pos = zeros(1,N); %replace with initial position

figure(2)
plot(H); title('Poisson Band-Pass Filter Coefficients');

% Display coefficients in a C++ array format for implementing on hardware
